...
TEST_CASE("Input text widget single line with callback", "[fast][InputTextWidget][visual]") {
    auto renderer = ImageRenderer::create();

    auto widget = new InputTextWidget("Single line input text:", "Test", true, [](auto text) {
        std::cout << "Text updated to: " << text << std::endl;
    });
